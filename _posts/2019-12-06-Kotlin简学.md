...
<font size=5 color=Red>G.如果你不希望你的类有一个公有构造函数，你需要声明一个带有非默认可见性的空的主构造函数：</font>
```
...
# 8.扩展
<font size=5 color=Red>A.扩展是静态解析的,扩展不能真正的修改他们所扩展的类。通过定义一个扩展，你并没有在一个类中插入新成员， 仅仅是可以通过该类型的变量用点表达式去调用这个新函数。</font>
```
fun Example.printFunctionType() { println("Extension function") }
```
<font size=5 color=Red>B.可空接收者,可以为可空的接收者类型定义扩展</font>
```
...
```
<font size=5 color=Red>C.扩展属性,由于扩展没有实际的将成员插入类中，因此对扩展属性来说幕后字段是无效的。这就是为什么扩展属性不能有初始化器。他们的行为只能由显式提供的 getters/setters 定义</font>
```
...
<font size=5 color=Red>A.向编译器解释这种情况。这称为声明处型变
...
<font size=5 color=Red>F.伴生对象</font>
```
...
<font size=5 color=Red>C.可观察属性 Observable</font>
```
...
<font size=5 color=Red>M.从 lambda 表达式中返回一个值</font>
...
<font size=5 color=Red>E.划分</font>
```
val numbers = listOf("one", "two", "three", "four")
